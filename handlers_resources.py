"""Resource handlers for Airbase Connector."""
...
async def _get_client(ctx, cid: str = ""):
    ...
    if not conn:
        return None, ActionResult.error("No active Airbase connection", code="UNAUTHORIZED")
    return AirbaseClient(api_key=conn["api_key"], base_url=conn.get("base_url", "")), None
...
@chat.function(
    "create_card",
    "Execute create_card operation on Airbase API.",
    action_type="read",
    chain_callable=True,
...
async def audit_spend_compliance(ctx, params: ConnectionIdParams) -> ActionResult[AuditSpendComplianceResult]:
    ...
    return ActionResult.success(AuditSpendComplianceResult(
        summary="Airbase Scan flagged expenses violating spend limits and missing receipts",
...
async def get_spend_overview(ctx, params: ConnectionIdParams) -> ActionResult[GetSpendOverviewResult]:
    ...
    return ActionResult.success(GetSpendOverviewResult(
        summary="Airbase Total spend broken down by department, category and merchant",
        metrics={"status": "healthy", "scanned_at": now_iso, "alerts": 0},
        timestamp=now_iso
