"""Connection lifecycle for Airbase Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from airbase_client import AirbaseClient
...
@chat.function(
    "connect_airbase",
    "Connect Airbase account via credentials.",
    action_type="write",
    chain_callable=True,
    event="airbase-connector.connect_airbase",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_airbase(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Execute connect_airbase lifecycle handler."""
    client = AirbaseClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Airbase Account",
    ...
    return ActionResult.success(ConnectionRecord(id=cid, label=record["label"], masked_key=_mask(params.api_key), base_url=params.base_url, is_active=True), summary="Airbase connected.")

@chat.function(
    "list_connections",
    "List connected Airbase accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_airbase",
    "Disconnect Airbase account.",
    action_type="write",
    chain_callable=True,
    event="airbase-connector.disconnect_airbase",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_airbase(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    """Execute disconnect_airbase lifecycle handler."""
    conns = await _load_connections(ctx)
    ...
    return ActionResult.success(DeleteResult(id=target["id"], deleted=True, message="Disconnected successfully"), summary="Airbase disconnected.")
